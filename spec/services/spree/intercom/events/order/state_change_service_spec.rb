...
RSpec.describe Spree::Intercom::Events::Order::StateChangeService, type: :service do
  ...
  let!(:event_service) { Spree::Intercom::Events::Order::StateChangeService.new(options) }
  let!(:intercom) { Intercom::Client.new(token: Spree::Config.intercom_access_token) }

  it 'is expected to define EVENT_NAME' do
    expect(described_class::EVENT_NAME).to eq('changed-order-state')
  end
