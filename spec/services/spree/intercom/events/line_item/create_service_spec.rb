...
RSpec.describe Spree::Intercom::Events::LineItem::CreateService, type: :service do
  ...
  let!(:event_service) { Spree::Intercom::Events::LineItem::CreateService.new(options) }
  let!(:intercom) { Intercom::Client.new(token: Spree::Config.intercom_access_token) }

  it 'is expected to define EVENT_NAME' do
    expect(described_class::EVENT_NAME).to eq('added-product')
  end
