...
RSpec.describe Spree::UserSerializer, type: :serializer do

  let!(:address) { create(:address) }
  let!(:user) { create(:user, bill_address_id: address.id) }
  let!(:user_without_address) { create(:user) }

  let!(:serialized_data_user) { ActiveModelSerializers::SerializableResource.new(user).serializable_hash }
  let!(:serialized_data_user_without_address) { ActiveModelSerializers::SerializableResource.new(user_without_address).serializable_hash }
  ...
  describe '#address_present?' do
    context 'when address is present' do
      let!(:serializer) { Spree::UserSerializer.new(user) }
    ...
    context 'when address is not present' do
      let!(:serializer) { Spree::UserSerializer.new(user_without_address) }
